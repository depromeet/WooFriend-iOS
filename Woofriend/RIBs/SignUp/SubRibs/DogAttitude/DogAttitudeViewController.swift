...
final class DogAttitudeViewController: BaseViewController, TagListViewDelegate, DogAttitudePresentable, DogAttitudeViewControllable {
    ...
    var dogAttitude: DogAttitude? = DogAttitude(character: [String](), Interest: [String]()) {
        didSet {
    ...
    private func bindUI() {
        mockInterest.forEach {
            interestView.addTag("#\($0)").onTap = { [weak self] in
                $0.isSelected.toggle()
                if $0.isSelected {
                    self?.dogAttitude?.Interest?.append($0.titleLabel?.text ?? "")
                        //?.append($0.titleLabel?.text)
                }
            }
        ...
        mockCharacter.forEach {
            characterView.addTag("#\($0)").onTap = { [weak self] in
                $0.isSelected.toggle()
                if $0.isSelected {
                    self?.dogAttitude?.character?.append($0.titleLabel?.text ?? "")
                }
            }
        ...
        nextButton.rx.tap.asSignal()
            .emit(onNext: { [weak self] b in
                ...
                if !(self.dogAttitude?.hasNilField() ?? true) {
                    self.listener?.nextAction()
                } else {

                    self.characterView.layer.borderColor = self.dogAttitude?.character?.isEmpty ?? true ?  #colorLiteral(red: 1, green: 0.4666666667, blue: 0.5294117647, alpha: 1).cgColor :  #colorLiteral(red: 0.8980392157, green: 0.8980392157, blue: 0.8980392157, alpha: 1).cgColor
                    self.interestView.layer.borderColor = self.dogAttitude?.Interest?.isEmpty ?? true ?  #colorLiteral(red: 1, green: 0.4666666667, blue: 0.5294117647, alpha: 1).cgColor :  #colorLiteral(red: 0.8980392157, green: 0.8980392157, blue: 0.8980392157, alpha: 1).cgColor
                }
            })
